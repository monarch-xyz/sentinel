...
describe("Scenario 1: Multi-Timeframe Whale Position Tracking", () => {
  ...
  function createMockContext(address: string): EvalContext {
    ...
    return {
      ...
      fetchState: vi.fn(async (ref: StateRef, timestamp?: number) => {
        ...
        if (timestamp >= NOW - TWO_DAYS_MS - 1000 && timestamp < NOW - 1000) {
  ...
  it("detects whale with >30% drop in 2d AND >40% drop in 7d (Bob)", async () => {
    ...
    const condition2d = buildPositionDropCondition("0xwhale2_bob_dumping", "2d", 0.3);
    const condition7d = buildPositionDropCondition("0xwhale2_bob_dumping", "7d", 0.4);

    const result2d = await evaluateCondition(
      condition2d.left,
      condition2d.operator,
      condition2d.right,
      context,
    );
    const result7d = await evaluateCondition(
      condition7d.left,
      condition7d.operator,
      condition7d.right,
      context,
    );
  ...
  it("detects whale with extreme exit (Eve)", async () => {
    ...
    const condition2d = buildPositionDropCondition("0xwhale5_eve_exiting", "2d", 0.3);
    const condition7d = buildPositionDropCondition("0xwhale5_eve_exiting", "7d", 0.4);
  ...
  it("does NOT trigger for steady whale (Alice)", async () => {
    ...
    const condition2d = buildPositionDropCondition("0xwhale1_alice_steady", "2d", 0.3);
    const condition7d = buildPositionDropCondition("0xwhale1_alice_steady", "7d", 0.4);
  ...
  it("does NOT trigger for accumulating whale (Carol)", async () => {
    ...
    const condition2d = buildPositionDropCondition("0xwhale3_carol_accumulating", "2d", 0.3);
    const condition7d = buildPositionDropCondition("0xwhale3_carol_accumulating", "7d", 0.4);
  ...
  it("handles mixed signals (Dave - up short-term, down long-term)", async () => {
    ...
    const condition2d = buildPositionDropCondition("0xwhale4_dave_volatile", "2d", 0.3);
    const condition7d = buildPositionDropCondition("0xwhale4_dave_volatile", "7d", 0.4);
