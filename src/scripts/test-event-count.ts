...
async function main() {
  ...
  if (!marketField || !userField) {
    ...
    if (!userField) {
      throw new Error(
        `Could not detect user field for ${eventType}. Set EVENT_USER_FIELD explicitly.`,
      );
    }
  }
  ...
  const filters: EventRef["filters"] = [{ field: chainField, op: "eq", value: chainId }];
